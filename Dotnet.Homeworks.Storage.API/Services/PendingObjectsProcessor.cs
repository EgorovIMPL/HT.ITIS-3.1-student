        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                foreach (var itemName in await _pendingImageStorage.EnumerateItemNamesAsync(stoppingToken))
                {
                    ...
                    if (imageS3File is null) continue;
                    ...
                    var removeItemAsync = await _pendingImageStorage.RemoveItemAsync(itemName, stoppingToken);
                    if (removeItemAsync.IsFailure)
                        _logger.LogInformation(removeItemAsync.Error);
                }
            }
            catch(Exception ex)
            {
